...
private extension [URL] {
    func paths(rootDirectory: URL, relativeTo: URL?) -> [String] {
        guard let relativeTo = relativeTo?.deletingLastPathComponent().absoluteString else { return self.map(\.absoluteString) }
        return self.map {
            $0.absoluteString.deletingPrefix(relativeTo)
        }
    }
}
